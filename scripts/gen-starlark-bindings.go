...
func genMapping(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		for i := range binding.argNames {
			...
			case "*github.com/lupes/delve/service/api.LoadConfig":
			...
			case "*github.com/lupes/delve/service/api.EvalScope":
...
func genDocs(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		fmt.Fprintf(&buf, "%s(%s) | Equivalent to API call [%s](https://godoc.org/github.com/lupes/delve/service/rpc2#RPCServer.%s)\n", binding.name, argNames, binding.fn.Name(), binding.fn.Name())
	}
...
func main() {
	...
	pkgs, err := packages.Load(cfg, "github.com/lupes/delve/service/rpc2")
	if err != nil {
	...
	packages.Visit(pkgs, func(pkg *packages.Package) bool {
		if pkg.PkgPath == "github.com/lupes/delve/service/rpc2" {
